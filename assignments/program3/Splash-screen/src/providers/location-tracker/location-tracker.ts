...
export class LocationTracker {
  ...
  constructor(public zone: NgZone,public geolocation: Geolocation,public data:DataProvider, public userService: UserService) {
    this.userService.getCurrentUser() //sets user to be current user
    .then(user => {
      this.user = user;
    }, err => console.log(err))
  }
  ...
  this.watch = 
    ...
    this.data.sendLocation(position).then(()=>{}).catch((p:any)=>console.log("error is "+p));
